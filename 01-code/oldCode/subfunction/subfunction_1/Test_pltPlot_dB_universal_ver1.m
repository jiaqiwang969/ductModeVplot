function [rpm,tsignal,xuhao]=Test_pltPlot_dB_universal_ver1(signal,fs,object,objectName,testTime,fname,save_directory,if_RI,type)
...
%----------------%%%%---------------------%%%%%---------------------
%% 
%对pulse进行调整，最后输出数组序列一致

Len1=min(diff(Pulse));

KKK=45;
solutime=1;
for H=1:KKK %每转一圈刷新一次网格，一个时间步！%按照RI_marker排序
%% 计算和安排相对位置关系
...
% 导入数据，对网格赋值
    rpm4av{solutime}(:,1) = signal(Pulse(H):Pulse(H)+Len1,object(1));%传感器B1
    rpm4av{solutime}(:,2) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(2));
    rpm4av{solutime}(:,3) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(3));
    rpm4av{solutime}(:,4) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(4));
    rpm4av{solutime}(:,5) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(5));
    rpm4av{solutime}(:,6) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(6));
    rpm4av{solutime}(:,7) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(7));
    rpm4av{solutime}(:,8) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(8));
    rpm4av{solutime}(:,9) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(9));
    rpm4av{solutime}(:,10) = signal((point_B1_C1_interval+Pulse(H)):(point_B1_C1_interval+Pulse(H)+Len1),object(10));%传感器C1
    solutime=solutime+1;
end

temp=zeros(size(rpm4av{1}));
for k=1:KKK
    temp=temp+rpm4av{k};
end
RPM_av=temp/KKK;
...
% 导入数据，对网格赋值
    rpm{solutime}(:,1) = signal(Pulse(H):Pulse(H)+Len1,object(1));%传感器B1
    rpm{solutime}(:,2) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(2));
    rpm{solutime}(:,3) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(3));
    rpm{solutime}(:,4) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(4));
    rpm{solutime}(:,5) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(5));
    rpm{solutime}(:,6) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(6));
    rpm{solutime}(:,7) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(7));
    rpm{solutime}(:,8) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(8));
    rpm{solutime}(:,9) = signal((point_B1_R1_interval+Pulse(H)):(point_B1_R1_interval+Pulse(H)+Len1),object(9));
    rpm{solutime}(:,10) = signal((point_B1_C1_interval+Pulse(H)):(point_B1_C1_interval+Pulse(H)+Len1),object(10));%传感器C1
 ...
 %%
     ...
     tsignal.surfaces(solutime).v(1,:,:)=rpm{solutime}-RPM_av;%根据键向信号判读
...
if if_RI ==0
NAME = [strrep(char(fname),type,'-'),'-blade-',date];  %存储文件夹
else
