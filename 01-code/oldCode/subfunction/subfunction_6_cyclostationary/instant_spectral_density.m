function St=instant_spectral_density(x,y,T,max_tau)
...
if nargout==0  
  [r,c]=size(St);
  dispSt=St(1:r,1:floor(c));
  dispSt=fftshift(dispSt);
  xl=(1:floor(c))-1;
  yl=(1:r)-floor(r/2)-1;

  xl=xl/c;yl=yl/r;
  figure(gcf+1);
  contour(xl,yl,abs(dispSt));
  title('instant spectral density');
  xlabel('t');
  ylabel('f ');
end
