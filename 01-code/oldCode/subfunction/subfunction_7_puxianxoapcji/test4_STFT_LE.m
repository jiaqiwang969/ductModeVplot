% Test 4 synthesized signal by bearing signal + sum of sinusoidal signals
...
fn = 3000;
fp = 120;
NN = round(10000*fp/fs);
var_tao = 0.04;
fr = 20;
s = create_bearing_signal(NN,A,fs,fn,fp,var_tao,fr);
N = length(s);
ratio = 0; %信噪比,相对于冲击信号的峰值而言
ratio = 10^(ratio/20);
amp = std(s)/ratio;
randn('state',0);
noise = amp*randn(1,N);
...
fg = 180;
for i_period = 1:1:20
    periodic_sig = periodic_sig +(5*amp/i_period)*sin(2*pi*fg*i_period*n);
end
data = s + noise + periodic_sig;
x = (data - mean(data))/std(data);
x = x';

%band-pass filtration

f=[2*0.85*fn/fs 2*0.9*fn/fs 2*1.1*fn/fs 2*1.15*fn/fs];
A=[0 1 0];
rp=0.153;
rs=16.92;
devp=1-10^(-rp/20);
