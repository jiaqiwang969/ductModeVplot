...
function computeDMD_oneModeAnimation_shock(rpm,tsignal,save_directory,name,T)
    dt=T/length(rpm);
    for kk=1:length(rpm)
...
%% DMD reconstruction全模态 及其 单个模态（除去背景）分析-并输出tecplot
     ...
     Tsignal.varnames= tsignal.varnames;
        ...
        for kk=1:length(t)
            ...
            Tsignal.surfaces(kk).v(4,:,:)= reshape(tsignal.surfaces(kk).v(1,1:min(len),:),min(len),10); 
            Tsignal.surfaces(kk).v(5,:,:)= reshape(tsignal.surfaces(kk).v(1,1:min(len),:),min(len),10)-reshape(abs(Xdmd_background(:,kk)),min(len),10); 
            
            Tsignal.surfaces(kk).solutiontime=kk;
        end        
        mat2tecplot(Tsignal,[save_directory,'\',name,'DMD-refine-ver4-d','.plt']);
end