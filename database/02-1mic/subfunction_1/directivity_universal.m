%directivity_universal.m  by wjq 2018-06-24
%by wjq
%  

function [rotor_speed]=directivity_universal(signal,fs,Scale_data,object,nk,objectName,testTime,fname)
    ...
    Fk=[1/2,1,2,3];
    thita=linspace(0,pi/2,nk);
    for k=1:length(Fk)
        xf(:,k)=max(abs(temp_freq(floor(rotor_speed/60*29*Fk(k)/(fs/length(signal)))+[floor(-2/(fs/length(signal))):floor(2/(fs/length(signal)))],:)))';
        subplot(2,2,k)
        polar(thita,xf(:,k)','r');hold on
        polar(-thita,xf(:,k)','r');
        title([num2str(Fk(k)),'*BPF'],'FontSize',14);

    end
suptitle({[testTime,'-压气机进口指向性试验'];[fname,'-转速: ',num2str(rotor_speed),'-采样率：',num2str(fs)]})
end
