...
function [rpm,tsignal,xuhao,Rotor_Speed]=pltPlot_dB_universal(signal,fs,object,objectName,testTime,fname,save_directory,if_RI,type)
...
Varnames=['Tecplot-(',strrep(char(fname),type,'-'),')'...
    num2str(round(mean(Rotor_Speed))),'rpm-',num2str(fs)];

tsignal.Nvar=4;     
tsignal.varnames={'x','y','z',Varnames};
mat2tecplot(tsignal,[save_directory,'\','Tecplot-(',strrep(char(fname),type,'-'),')'...
    num2str(round(mean(Rotor_Speed))),'rpm-',num2str(fs),'.plt']);
