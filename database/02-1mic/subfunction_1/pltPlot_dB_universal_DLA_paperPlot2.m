function [RPM1,tsignal,Pulse_blade,T,signal_X]=pltPlot_dB_universal_ver1(signal,fs,object,objectName,testTime,fname,save_directory,if_RI,type)

[Pulse,Rotor_Speed] = keyRotation_RealTime(signal(1:end,end),fs); %通过键向信号获取转速信息.去头去尾      
N_blade=29;%添加一个功能-3或4个叶道为一组-增大数据组数
...
hold on
plot(Pulse_RI_high1,S_data(Pulse_RI_high1),'MarkerSize',25,'Marker','.','LineStyle','none','Color',[1 1 0])
xlim(axes1,[Pulse_RI(2) Pulse_RI(6)]);

%pause
lll=1:length(signal(:,end));
hold on
plot(Pulse_divide+300,S_data(Pulse_divide),'MarkerSize',25,'Marker','.','LineStyle','none','Color',[1 0 0])
plot(lll+600,signal(:,end)*8000+Threshold-25000)
