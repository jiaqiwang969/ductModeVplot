function pltPlot_dB_universal_stallIncepion(signal,fs,object,objectName,testTime,fname,save_directory,if_RI,type)
...
% plot([1 204800],[Threshold Threshold],'LineStyle','--');
% 
% plot(point_B1_R1_interval+Pulse,Threshold*ones(size(Pulse)),'<');
...
%      tsignal.surfaces(solutime).z=YY;    %size 3x3
       tsignal.surfaces(solutime).x=Y;    %size 3x3 
       tsignal.surfaces(solutime).y=X;    %size 3x3
       tsignal.surfaces(solutime).z=Z;    %size 3x3
